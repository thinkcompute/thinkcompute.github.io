...
- Transform a problem into a recursive formulation (base case + recursive step)
...
# Trace the execution
print(f"multiply(3, 4) = 3 + multiply(3, 3)")
print(f"              = 3 + 3 + multiply(3, 2)")
print(f"              = 3 + 3 + 3 + multiply(3, 1)")
print(f"              = 3 + 3 + 3 + 3 + multiply(3, 0)")
print(f"              = 3 + 3 + 3 + 3 + 0")
print(f"              = {multiply(3, 4)}")
```
...
```{code-cell} python
def power(a, n):
    """Compute a raised to the power n using recursion."""
    if n == 0:
        return 1
    else:
...
- **Recursive thinking**: How to transform a problem into base case + recursive step
- **Text files**: Reading and writing with `open()`, `read()`, `write()`, and the `with` statement
- **File modes**: `"r"` for reading, `"w"` for writing, `"a"` for appending
- **CSV files**: Using `csv.reader()`, `csv.writer()`, `csv.DictReader()`, and `csv.DictWriter()`
- **JSON files**: Using `json.load()`, `json.dump()`, `json.loads()`, and `json.dumps()`
- **Choosing file formats**: TXT for plain text, CSV for tables, JSON for structured data
- **Context managers**: Using `with open(...) as file:` for safe file handling
